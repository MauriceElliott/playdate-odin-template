...
Button_Callback_Proc :: #type proc "c" (
	button: Buttons,
	down: b32,
	_when: u32,
	user_data: rawptr,
...
Api_System_Procs :: struct {
	realloc:                       proc "c" (ptr: rawptr, size: u32) -> rawptr,
	...
	add_menu_item:                 proc "c" (
		title: cstring,
		callback: Menu_Item_Callback_Proc,
		userdata: rawptr,
	) -> ^Menu_Item,
	add_checkmark_menu_item:       proc "c" (
		title: cstring,
		value: i32,
		callback: Menu_Item_Callback_Proc,
		userdata: rawptr,
	) -> ^Menu_Item,
	add_options_menu_item:         proc "c" (
		title: cstring,
		option_titles: [^]cstring,
		options_count: i32,
		callback: Menu_Item_Callback_Proc,
		userdata: rawptr,
	) -> ^Menu_Item,
	...
	// 2.4
	set_button_callback:           proc "c" (
		callback: Button_Callback_Proc,
		user_data: rawptr,
		queue_size: i32,
	),
	set_serial_message_callback:   proc "c" (callback: Serial_Message_Callback_Proc),
	va_format_string:              proc "c" (
		out_string: ^cstring,
		format: cstring,
		va_list: c.va_list,
	) -> i32,
	parse_string:                  proc "c" (str: cstring, format: cstring, args: ..any) -> i32,
	delay:                         proc "c" (milliseconds: u32),

	// 2.7
	get_server_time:               proc "c" (callback: proc "c" (time: cstring, err: cstring)),
	restart_game:                  proc "c" (launch_args: cstring),
	get_launch_args:               proc "c" (out_path: ^cstring) -> cstring,
	send_mirror_data:              proc "c" (command: u8, data: rawptr, len: c.int) -> b32,

	// 3.0
	get_system_info:               proc "c" () -> ^PDInfo,
}
