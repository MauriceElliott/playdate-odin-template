...
Sprite_Collision_Filter_Proc :: #type proc "c" (
	sprite, other: ^Sprite,
) -> Sprite_Collision_Response_Type
// =================================================================
...
Api_Sprite_Procs :: struct {
	...
	set_bounds:                      proc "c" (sprite: ^Sprite, bounds: PDRect),
	get_bounds:                      proc "c" (sprite: ^Sprite) -> PDRect,
	...
	set_collide_rect:                proc "c" (sprite: ^Sprite, collide_rect: PDRect),
	get_collide_rect:                proc "c" (sprite: ^Sprite) -> PDRect,
	clear_collide_rect:              proc "c" (sprite: ^Sprite),

	// caller is responsible for freeing the returned array for all collision methods
	set_collision_response_function: proc "c" (
		sprite: ^Sprite,
		filter: Sprite_Collision_Filter_Proc,
	),
	check_collisions:                proc "c" (
		sprite: ^Sprite,
		goal_x, goal_y: f32,
		acutal_x, actual_y: ^f32,
		length: ^c.int,
	) -> [^]Sprite_Collision_Info,
	move_with_collisions:            proc "c" (
		sprite: ^Sprite,
		goal_x, goal_y: f32,
		acutal_x, actual_y: ^f32,
		length: ^c.int,
	...
	query_sprites_in_rect:           proc "c" (
		x, y, width, height: f32,
		length: ^c.int,
	) -> [^]^Sprite,
	query_sprites_along_line:        proc "c" (
		x_1, y_1, x_2, y_2: f32,
		length: ^c.int,
	) -> [^]^Sprite,
	query_sprite_info_along_line:    proc "c" (
		x_1, y_1, x_2, y_2: f32,
		length: ^c.int,
	) -> [^]Sprite_Query_Info,
	...
	// 2.7
	set_tilemap:                     proc "c" (sprite: ^Sprite, tilemap: ^Tile_Map),
	get_tilemap:                     proc "c" (sprite: ^Sprite) -> ^Tile_Map,
}
// =================================================================
